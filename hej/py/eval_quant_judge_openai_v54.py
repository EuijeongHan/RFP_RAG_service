"""
입찰메이트 RAG — LLM-as-a-Judge 재채점 (OPENAI 3개 시나리오만)
=================================================================
담당 : Generation 파트 (한의정)

[목적]
  OPENAI 생성을 gpt-5.4-mini로 재생성했으므로, judge도 gpt-5.4-mini로
  OPENAI 3개 시나리오만 재채점한다. 나머지 9개 시나리오 점수는
  기존 quantitative_scores_all.csv를 그대로 보존한다(머지 단계에서 처리).

  ⚠️ 공정성 한계(발표에 명시): 결과표는 OPENAI 3개만 gpt-5.4-mini judge,
     나머지 9개는 기존 judge로 채점된 값이 섞인다. 엄밀한 12개 동일-judge
     비교는 아니다.

[Judge]  gpt-5.4-mini (OpenAI) — 비동기 병렬 + 지수 백오프 재시도
[입력]   e2e_all_comparison_v12.csv   (OPENAI 3컬럼 모두 채워진 합본)
[출력]   quantitative_scores_openai_v54.csv  (OPENAI 3개 시나리오 점수만)

[실행]
  export OPENAI_API_KEY=...
  python eval_quant_judge_openai_v54.py
"""
...
# ======================================================================
# 경로 — Colab/Drive 기준. 실제 환경에 맞게 수정.
# ======================================================================
_RESULT_DIR  = Path('/mnt/gukrul/dataset/eval_results')
_INPUT_PATH  = _RESULT_DIR / 'generation' / 'e2e_all_comparison_v12.csv'
_OUTPUT_PATH = _RESULT_DIR / 'generation' / 'quant' / 'quantitative_scores_openai_v54.csv'

# ======================================================================
# 채점 대상 — OPENAI 3개 시나리오만
# ======================================================================
_SCENARIOS = [
    'ans_kure_openai',
    'ans_koe5_openai',
...
# ======================================================================
# Judge 프롬프트 — 기존 eval_quant_judge1_all_async.py와 동일(정합 유지)
# ======================================================================
_JUDGE_PROMPTS = {
    'faithfulness': (
...
def _is_row_complete(row: pd.Series) -> bool:
    """relevance 컬럼 기준으로 행이 정상 채점됐는지 확인 (OPENAI 3개만)"""
...
async def run_all_async():
    _logger.info(f'[Judge-OPENAI] 시작 | Judge={_JUDGE_1_MODEL} | 대상=OPENAI 3개 시나리오')
    df = pd.read_csv(_INPUT_PATH)
    _logger.info(f'[Judge-OPENAI] 입력: {len(df)}행 × {len(_SCENARIOS)}개 시나리오')
    ...
    if _OUTPUT_PATH.exists():
        try:
            ...
            _logger.info(
                f'[Judge-OPENAI] 체크포인트: 정상 {len(checkpoint_rows)}행 재사용'
                + (f' | NaN {n_invalid}행 재채점' if n_invalid > 0 else '')
            )
        except Exception as e:
            _logger.warning(f'[Judge-OPENAI] 체크포인트 파싱 실패 → 전체 재실행: {e}')

    pending = [(idx, row) for idx, row in df.iterrows()
               if row['question'] not in completed_questions]
    _logger.info(f'[Judge-OPENAI] 신규 채점 대상: {len(pending)}행')

    if not pending:
        _logger.info('[Judge-OPENAI] 모든 행 채점 완료 상태.')
        return
    ...
    for idx, row in tqdm(pending, total=len(pending), desc='[Judge] 채점 중'):
        ...
        gt      = row.get('ground_truth_answer', None)  # v12엔 없음 → corr/recall N/A
        ...
        if len(results) % 50 == 0:
            ...
            _logger.info(f'[Judge-OPENAI] 체크포인트 저장: {len(results)}행')

    result_df = pd.DataFrame(results)
    result_df.to_csv(_OUTPUT_PATH, index=False, encoding='utf-8-sig')
    _logger.info(f'[Judge-OPENAI] ✅ 저장 완료: {_OUTPUT_PATH}')

    print('\n📊 OPENAI 재채점 요약 (avg 기준, gpt-5.4-mini judge)')
